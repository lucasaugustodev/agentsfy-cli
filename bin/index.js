...
auth.command("config")
    ...
    if (config.default_project)
        console.log(chalk.dim("Project:"), config.default_project);
});
// ── Agents ──
...
files.command("ls")
    ...
    try {
        let p = dirPath;
        if (p.includes("Program Files/Git/"))
            p = p.replace(/.*Program Files\/Git/, "");
        if (!p.startsWith("/"))
            p = "/" + p;
...
webhooks.command("list")
    ...
    try {
        ...
        if (!webhooks.length) {
            console.log(chalk.dim("No webhooks."));
            return;
        }
